...
namespace kinematics_20160720
{
	...
	static class TimelineConnector
	{
        ...
        public static void send_file_command(byte[] command_with_data)
        {
            using (var pipeclient = new NamedPipeClientStream(".", "timeline", PipeDirection.InOut, PipeOptions.None))
            {
                try { pipeclient.Connect(timeout_connect); }
                ...
                {
                    tr("try to writeline: ");
                    pipeclient.Write(command_with_data, 0, command_with_data.Length);
                    pipeclient.Flush();
                    tr("write ok");
                    tr("try to read");
                    tr("end");
                }
                catch (Exception ex) { tr("exception: " + ex.Message); }
                pipeclient.Close();
                tr("pipeclient closed");
            }
        }
